    ...
    <section className="min-h-screen flex flex-col justify-center items-center text-center px-6 transition-colors duration-300">
      <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
        ...
        <div className="mt-6 flex justify-center gap-4">
          <a href="https://github.com/Ajaylakhara" target="_blank" rel="noopener noreferrer" className={`text-2xl ${isDark ? "text-gray-500 hover:text-gray-200" : "text-gray-300 hover:text-gray-900"} transition-colors duration-300`}>
            GitHub
          </a>
          <a href="https://www.linkedin.com/in/ajay-lakhara-9159b0190/" target="_blank" rel="noopener noreferrer" className={`text-2xl ${isDark ? "text-gray-400 hover:text-gray-200" : "text-gray-200 hover:text-gray-900"} transition-colors duration-300`}>
            LinkedIn
          </a>
        </div>
      ...
      <div className={`w-full relative overflow-hidden ${isDark ? "border-black" : "border-white"} border-y py-4 mt-12`}>
        <div className="flex w-max animate-marquee gap-6">
          {[...skills, ...skills].map((skill, index) => (
            <div key={index} className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm ${isDark ? "bg-black text-white" : "bg-white text-black"} shadow-md`}>
              <img src={skill.img} alt={skill.name} width="18" height="18" className="text-transparent" />
              {skill.name}
            </div>
          ))}
        </div>
...
// The rest of the files will be filled in next update blocks.
